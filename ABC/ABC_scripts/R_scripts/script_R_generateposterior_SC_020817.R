...
outfile="Posteriors500_Pool-rob-pet_SC_hetero_hetero_020817_"
...
for(i in 1:50){ # au lieu de 1:251
  tmp.stat=rbind(tmp.stat, matrix(scan(paste("./2ndarycont_birdlogin/rob-pet_2ndarycont_Nhetero_Mhetero_v1_",i,"/ABCstat.txt",sep=""), skip=2), byrow=T, ncol=20)[,ss])
  tmp.parameters=rbind(tmp.parameters, matrix(scan(paste("./2ndarycont_birdlogin/rob-pet_2ndarycont_Nhetero_Mhetero_v1_",i,"/priorfile",sep=""), skip=1), byrow=T, ncol=17))
