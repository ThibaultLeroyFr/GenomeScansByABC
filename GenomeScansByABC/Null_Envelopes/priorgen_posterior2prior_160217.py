...
import sys
n1, n2, nA, tau, M1, M2, shape1, shape2, RatioTsmallTsplit = [], [], [], [], [], [], [], [], []
...
\t\tRatioTsmallTsplit: prior for the ratio between Tsmall (e.g. Tsc) & Tsplit (the time of speciation\n\
...
for i in sys.argv:
	if "=" in i:
		...
		if(i[0]=="RatioTsmallTsplit"):
			RatioTsmallTsplit.append(float(i[1]))
		if(i[0]=="RatioTsmallTsplit"):
			RatioTsmallTsplit.append(float(i[1]))
		if(i[0]=="M1"):
			M1.append(float(i[1]))
...
for i in range(nreps):
	...
	RatioTsmallTsplitprior=uniform(RatioTsmallTsplit[0],RatioTsmallTsplit[1])   # ici je considère la posterior du ratio (p.ex. TSC/Tsplit pour SC)
	Tsmall=Tsplit*RatioTsmallTsplitprior
